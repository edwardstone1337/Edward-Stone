...
- **Cursor chat message on the Kaomoji strip** (`index.html`): the existing `initCursorChat` feature (`assets/js/dev-projects/cursor-chat.js`, already live on the homepage and resume) gains a trigger showing "Take one with you" over the strip. Scoped to the text column and button rather than the whole section: the preview is an iframe, and pointer moves inside it never reach the parent document, so a section-wide trigger left the bubble parked at the iframe's edge up to 500px adrift of the real cursor. Leaving the column fires `mouseleave` and hides it cleanly. No change to the cursor-chat module itself; the bubble is a fixed dark chip independent of the strip's light theme, and the feature already gates itself to fine-pointer devices and respects `prefers-reduced-motion`.
