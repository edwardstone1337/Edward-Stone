...
| `cursor-chat.js` | ES6 module | Desktop-only speech-bubble that follows the cursor, triggered by hover or scroll position (`initCursorChat(config)` with a `triggers` array). Header comment states it bails on prod and on touch/keyboard devices, but the file itself has **no `is-prod` gate** — it is intentionally live on production. Loaded on `index.html` (dynamic `import()`, ten triggers including the Kaomoji strip's), `personal.html` (dynamic `import()`, one trigger — the Kaomoji strip's), `resume.html` (static `import`), and `index.html` (dynamic `import()`, five triggers including the Kaomoji strip's and the new floating-testimonials gutter hotzones). Reads `trigger.message` at hover-fire time rather than at init, which is what lets `kaomoji-strip.js` (below) swap a trigger's message after the fact just by mutating the object it handed back |
| `kaomoji-strip.js` | ES6 module | Owns the kaomoji.click product strip's markup **and** behaviour in one place — `initKaomojiStrip(mountSelector)` finds the placeholder element, replaces it (not fills it, so no wrapper `<div>` enters the layout) with the rendered `<section class="dp-strip dp-strip--flipped dp-strip--kaomoji">`, and wires the strip's own behaviour: the codebase's first `window.addEventListener('message', ...)` receiver, validating `event.origin` and `event.data.type === 'kaomoji-copy'` before swapping the returned `cursorTrigger.message` (held by reference) from "Try clicking one" to "Great choice"; and a `mouseenter`/`mouseleave` bridge on the preview `<iframe>` that fakes section-level `mouseleave`/`mouseenter`, since pointer events inside the iframe never reach the parent document and would otherwise leave the cursor-chat bubble hanging at the iframe boundary. Returns `{ section, cursorTrigger }`, or `null` if the mount selector matches nothing. Deliberately does not call `initCursorChat` itself — the caller composes `cursorTrigger` into its own `initCursorChat()` call, and must mount the strip *before* that call, since `cursor-chat.js` resolves trigger selectors once at init and silently skips ones matching nothing. Styles: `.dp-strip`/`.dp-strip--kaomoji` in `dev-styles.css`, tokens in `dev-tokens.css`; also needs `cursor-chat.css` loaded on the page for the bubble. Loaded on `index.html`, `personal.html`, and `index.html`. Full API and the ordering requirement: `docs/strip-branding-spec.md` §7 |
| `floating-testimonials.js` | ES6 module | Drifts short quotes up through the empty gutters either side of a centred element, fading in and out as they rise — `initFloatingTestimonials(config)` plus the exported `PLANNER_TEACHER_QUOTES` fixture (real teacher quotes from the wall of love on `case-studies/planner.html`, filtered to short ones). Built for the homepage flagship, where the Planner prototype on its own only reads as "a kanban board." Deliberately decorative: `pointer-events: none` throughout so it can never steal a click from the interactive prototype beside it; `aria-hidden` on the layer, so a screen reader isn't read twenty-odd quotes it didn't ask for (the same testimonials are real content on `case-studies/planner.html`); returns `null` and renders nothing under `prefers-reduced-motion`. Measures the layer from `document.documentElement.clientWidth` rather than `100vw` — `100vw` includes the scrollbar, which would push the page into horizontal scroll. Self-disables when the measured gutter is too narrow for even the minimum bubble width, rather than hard-coding a breakpoint. Renders transparent `.dp-float-hotzone` gutter zones (sized to stop exactly at the board edge) so cursor-chat has something to attach to, since the bubbles themselves are `pointer-events: none` and moving. Collision avoidance checks real bounding boxes rather than `top` percentages, because bubbles hug their text (`width: max-content`) and so vary wildly in height — a percentage gap that clears "Love it!" won't clear a four-line quote. Loaded on `index.html` |
| `snail.js` | ES6 module | Pixel-art snail that crawls clockwise laps around any element's border, dragging a slime trail — `initSnail(host, { size, speed, mono, pathTarget })`. Ported from Edward's Claude Design component and extended from a single top-edge crawl to a full perimeter lap. Upright along the top, sideways down the right, upside down along the bottom, rounding each corner along its actual border-radius; click it and it retracts into its shell and stops, click again and it carries on. The lap is CSS Motion Path: the module measures the traced element and writes an `offset-path: path(…)` rounded rect, `offset-anchor: 50% 100%` puts the snail's FEET on the line, and `offset-rotate: auto` turns it to follow the tangent — that is what rounds the corners and flips it underneath. The path is baked from measured pixels, so it is rebuilt via a `ResizeObserver` on the traced element; a window resize listener alone would miss the Planner's board changing height when you switch tabs. The snail rides the OUTSIDE of the border, so a traced element with `overflow: hidden` clips it — mount on a wrapper and pass the bordered element as `pathTarget` (`index.html` does exactly this, since `.pl-frame` hides its overflow, and waits on a `MutationObserver` because the frame only exists once the Planner has mounted). Browsers without `offset-path` fall back to the original top-edge-only crawl, feature-detected via `CSS.supports`. Decorative: `aria-hidden`, not focusable — a focusable target that slides around the screen is worse than no target at all — and under `prefers-reduced-motion` it parks with no lap, no walk cycle and no trail, but stays clickable. Loaded on `index.html` and `dev/design-system.html` (section 6q) |
...
A self-contained "prototype kit" (14 files) built for the Planner prototype — `projects/planner.html` (full page, still prod-gated) and the homepage, where it is the flagship: `index.html` mounts it into `.dp-flagship__widget` with `{ chrome: false }` above the fold, poster-swapped in on successful `import()`, live on **all** environments including prod. The pre-redesign homepage embedded it lower down the page as `.dp-case-promo__widget`; that markup now only exists in the archived `dev/old-index-2026-08-24.html`. Designed so a future case-study prototype can reuse the board/card/row/drawer machinery without forking it. Full design/decision log: `docs/superpowers/specs/2026-08-23-planner-prototype-design.md`.
