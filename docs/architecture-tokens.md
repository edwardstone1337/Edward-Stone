...
- **Flip 7**: `--dp-strip-flip7-band` (the full-bleed teal surface), `--dp-strip-flip7-bg` (the cream phone on it), `--dp-strip-flip7-title-*`, `--dp-strip-flip7-accent*` (CTA), `--dp-strip-flip7-phone-radius` / `-media-ratio` / `-media-max-h`, plus the orb and badge tokens it keeps mapped but does not render. Authored in hex, carrying flip7scorecard.com's palette verbatim — this strip was previously the only one written in `lch()`; that is no longer the case and every strip is now hex/rgba. It is the second **light** strip after Kaomoji, and the only full-bleed one
...
| `.dp-strip`, `-inner`, `-content`, `-media`, `-title`, `-description`, `-badge(s)`, `-logo`, `-orbs`, `-skeleton`, `--compact`, `--flipped`, `--interactive`, `--scp`, `--flip7`, `--kaomoji` | ~1657 | Branded product strip — the most systematised pattern in the codebase. Brand a new one with **tokens only, no new CSS**: see `docs/strip-branding-spec.md`. `--kaomoji` and `--flip7` are the two `color-scheme: light` opt-outs, and `--flip7` is the one variant that also carries real CSS (full-bleed `100vw` band, `100svh`, orbs hidden, media slot reshaped to a phone) because it changes the strip's *shape*, not just its palette. Homepage-only today; every strip except Kaomoji is `data-prod-hide` |
