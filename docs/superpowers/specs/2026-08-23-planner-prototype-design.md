...
2. **Embed**: Inline compact mount into the homepage's `.dp-case-promo` section in `index.html` (the decision originally named `case-studies/planner.html`; the embed shipped on `index.html` instead) using a compact token variant (~200–220px columns). Never `transform: scale()` (brief §9 gotcha). Full-size page: `projects/planner.html` (already scaffolded, prod-gated).
...
- Round 11: the homepage embed goes live on production. `.dp-case-promo__widget` loses `data-prod-hide` and its mount script loses the `isProd` guard, so the prototype now runs for real visitors instead of dev/local only. `projects/planner.html`'s own page-level gate is deliberately left in place — the embed mounts `planner.js` directly rather than iframing that page, so the two ship independently. The `planner.png` screenshot that stood in on prod is retained but repurposed as the `<noscript>` fallback (the widget needs JS), and `.dp-case-promo__media` drops its `.is-prod` display rule accordingly.
