...
- `index.html` is hero, logo bar, colleague testimonials, blue "Let's talk" panel (homepage v3, 2026-08-25). The live Planner prototype led this page for one day as v2 and has since moved to `case-studies/planner.html`, taking the snail and the floating teacher quotes with it; the kaomoji strip moved to `personal.html`. The ticker, side quests, toolbox and product strips remain gone. "Read the case study" in the hero is the **only** link to the work in the page body, by design: everything else routes through the Case Studies nav dropdown. Archived predecessors: `dev/old-index-2026-08-24.html` (pre-v2). Design records: `docs/superpowers/specs/2026-08-25-homepage-v3-design.md`, and `.../2026-08-24-homepage-v2-design.md` for the v2 step
...
- `case-studies/planner.html` hosts the live Planner prototype directly under the TL;DR, with the snail lapping its frame and the floating teacher quotes in the gutters. Its `#wall-of-love` is **not** dead markup: `floating-testimonials.js` visually hides it while the float layer runs and shows it when there is no gutter to drift in (below roughly 1356px, including 1024), so the two are alternatives, never both on or both off
