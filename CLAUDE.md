...
**Strips** (`.dp-strip`) default to dark via `color-scheme: dark` regardless of page theme. **Two strips opt out** to `color-scheme: light` and carry their product's own light palette, so each section and its embedded preview read as one product surface: `.dp-strip--kaomoji` (kaomoji.click's warm neutral) and `.dp-strip--flip7` (flip7scorecard.com's teal/cream/navy). Any further strip going light needs the same opt-out plus light values for every `--dp-strip-*` token it maps. `.dp-strip--flip7` also breaks the mould structurally: it is the only full-bleed strip (`100vw`, `min-height: 100svh`, no radius), it hides its orbs, and its heading is the one piece of strip text in the codebase that does **not** meet WCAG AAA — a deliberate, documented exception. See `docs/strip-branding-spec.md` §5 before copying anything from it. `theme-toggle.js` broadcasts `{ type: 'theme-change' }` to preview iframes via `postMessage`, but no preview iframe has ever listened for it — a broadcast with no consumer. The one exception is unrelated to theming: the Kaomoji preview posts `{ type: 'kaomoji-copy' }` (no display text) to its parent after a successful clipboard copy; `index.html` validates `event.origin` before swapping its own cursor-chat wording — the iframe never supplies the text that gets rendered.
