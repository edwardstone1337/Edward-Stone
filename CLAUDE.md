...
- On prod the nav shows a single **Case Study** link, pointing straight at `case-studies/planner.html`, plus Resume; Projects, Personal, Gallery and About are `prodHide`d in `nav-component.js`, and the snake game is not loaded at all. The site is deliberately leading with one case study for now (2026-09-06), so the four-item Case Studies dropdown is prod-only-hidden rather than removed: off prod the full dropdown still renders from `CASE_STUDIES`. Personal was unhidden in v3 when the kaomoji strip left the homepage, then hidden again in the same 2026-09-06 change
- The "Read another case study" section (`case-study-read-more.js`) is **suppressed on prod** for the same reason: every link it renders points at a case study the prod nav no longer offers. It still renders off prod. The other three case studies remain live, indexable and in `sitemap.xml` — they are unlinked on prod, not unpublished
